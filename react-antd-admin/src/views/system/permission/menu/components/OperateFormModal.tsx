  ...
  return (
    <ModalForm<API.MenuAddParams & API.MenuUpdateParams>
      ...
      onFinish={async values => {
        ...
        if (values.type === 2 && params.perms?.length >= 1) {
          const perms = (params.perms as unknown as string[][]).flatMap(n => n.at(-1));
          params.perms = permissionValues.filter(n => perms.some(m => n.includes(m as string))).join(',');
        }
        if (!params.openMode || params.openMode === 2) {
          params.openMode = 1;
        }
        if (detail?.id) {
        ...
        {({ type }) => {
          return type === 2 ? (
            <Form.Item name="perms" label="权限">
              <Cascader multiple={true} options={cascaderOptions} showCheckedStrategy={SHOW_CHILD}></Cascader>
            </Form.Item>
          ) : null;
        }}
      </ProFormDependency>
